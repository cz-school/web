...
export default {
	...
	methods: {
		...
		commodity() {
			uni.request({
				url: 'http://127.0.0.1:9999/api/v1/purchase/' + this.commodityId,
				method: 'POST',
				data: {},
				success: res => {
		...
		site() {
			// 获取数据
			uni.request({
				url: 'http://127.0.0.1:9999/api/v1/purchaseSite',
				method: 'POST',
		...
		payment() {
			uni.request({
				url: 'http://127.0.0.1:9999/api/v1/payment',
				method: 'POST',
				...
				success: res => {
					...
					if (code == '400') {
						// 生成订单
						uni.request({
							url: 'http://127.0.0.1:9999/api/v1/createorder',
							method: 'POST',
...
.purchase-location {
	...
	white-space: normal ！important;
	display: -webkit-box;
	-webkit-line-clamp: 1; /* 显示3行，为auto时不隐藏 */
	-webkit-box-orient: vertical;
}
.purchase-arrows {
	display: flex;
